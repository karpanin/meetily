                ...
                loop {
                    ...
                    match chunk {
                        Some(chunk) => {
                            ...
                            .await
                            {
                                Ok((transcript, confidence_opt, is_partial, speaker)) => {
...
/// Returns: (text, confidence Option, is_partial, speaker Option)
            ...
            match provider.transcribe(speech_samples, language).await {
                Ok(result) => {
                    ...
                    if cleaned_text.is_empty() {
                        return Ok((String::new(), result.confidence, result.is_partial, result.speaker));
                    }
                    ...
                    Ok((cleaned_text, result.confidence, result.is_partial, result.speaker))
                }
                Err(e) => {
                    error!(
